#!/usr/bin/env node
// Benchmarks the webgpu fused-encoder pipeline executor against today's
// per-pass kernel chaining and the generic pipeline executor, on the same
// two iterative-stencil workloads as scripts/benchmark-pipeline.mjs, against
...
// Prints a GitHub-markdown table plus raw JSON to stdout (progress goes to
// stderr, nothing is written to disk).
//
//   npm run make && node scripts/benchmark-pipeline-webgpu.mjs
//
// The workloads are benchmark-pipeline.mjs's jacobi and heat rows verbatim —
// same make(), same fp32-exact constants, same index-weighted checksums,
// same flat-buffer plain-JS oracle — redeclared inside the page function
// because gpu.js parses kernels via Function.prototype.toString and
// Playwright serializes the page function without closures. Methodology
// follows that script:
// - every mode's checksum is validated against the plain-JS oracle
//   (relative 1e-3) before timing; a mismatch aborts the run
...
// - median of 5 runs, the validation call serving as compile warmup; wall
//   clock only, since GPU timer queries report garbage on ANGLE Metal
// - every timed run is synced by reading back its own result (the per-pass
//   row's final toArray(), the pipeline rows' host-array return)
// - every run starts from the pristine grid (the per-pass row re-reads its
//   resident u0 handle, the pipeline rows re-sample their arguments), so no
//   run inherits a prior run's relaxation
//
// Cost accounting is deliberately tilted against the pipeline, as in the
// Node script: the per-pass row uploads its inputs ONCE at build while
// pipeline rows pay the argument snapshot + upload on EVERY call. Speedups
// are relative to the per-pass row.
...
// Runs inside the page in ONE session; must stay closure-free.
async function benchInPage() {
  const G = window.GPU;
  if (!G || !G.prototype || typeof G.prototype.createKernel !== 'function') {
    throw new Error('dist/gpu-browser.js did not expose GPU — run npm run make first');
  }
  if (!navigator.gpu) throw new Error('navigator.gpu absent');
  const adapter = await navigator.gpu.requestAdapter();
  if (!adapter) throw new Error('no WebGPU adapter (headless or blocklisted)');
  ...
  // Rows of a flat grid as a 2-D array, which is what a gpu.js kernel
  // indexes. subarray, not slice: these are views, so nothing is copied
  // here — the pipeline's own call-time snapshot is the copy being priced.
  ...
  // gpu.js hands back rows, the oracle hands back one flat array; both
  // shapes are walked rather than flattened. Index-weighted so a backend
  // that swept only part of the grid cannot match by luck.
  ...
  function jacobi() {
    ...
    const sweepSource = function (u, src) {
      ...
      // Dirichlet edge, copied through — keeps both ping-pong buffers
      // holding a correct edge without either being pre-filled
    ...
    return {
      ...
      js({ u0, q }) {
        // copied, not aliased: both buffers get the boundary because both
        // take a turn as the source
      ...
      async perPass(gpu, { u0, q }) {
        ...
        // reuses its own output buffer, so one instance cannot both read
        // the previous sweep and overwrite it
        ...
        // two identity uploads, not one called twice: the second call
        // would hand back the buffer it filled the first time
        ...
        const u0Handle = await upU(rows(u0, N));
        const qHandle = await upQ(rows(q, N));
        return {
          async run() {
            // sweep 0 reads the pristine u0 handle and writes kA's own
            // buffer, so every run starts from the same grid
            let t = u0Handle;
            for (let s = 0; s < SWEEPS; s++) t = await (s % 2 === 0 ? kA : kB)(t, qHandle);
      ...
      buildPipeline(gpu, { u0, q }) {
        ...
        return { shortcut: solve, run: async () => await solve(uRows, qRows) };
      },
    };
  }
  ...
  function heat() {
    ...
    return {
      ...
      // field energy: diffusion conserves the mean, so a mean-based
      // checksum would pass a backend that did nothing; the sum of squares
      // falls 17%
      reduce(out) {
      ...
      async perPass(gpu, { u0 }) {
        ...
        const u0Handle = await upload(rows(u0, N));
        return {
          async run() {
            let t = u0Handle;
      ...
      buildPipeline(gpu, { u0 }) {
        ...
        const uRows = rows(u0, N);
        return { shortcut: diffuse, run: async () => await diffuse(uRows) };
      },
    };
  }

  const MODES = [
    { label: 'webgpu per-pass', perPass: true },
    { label: 'pipeline generic', kind: 'generic', hook: pipeline => (pipeline._fusionDisabled = true) },
    { label: 'pipeline fused-encoder', kind: 'fused-encoder' },
  ];
  ...
  for (const workload of [jacobi(), heat()]) {
    ...
    // plain-JS oracle: reference only, computed once — the columns are all
    // webgpu
    const expected = workload.reduce(workload.js(inputs));
    log(workload.name, '/ plain-JS reference checksum:', expected.toFixed(6));
    for (const mode of MODES) {
      const gpu = new G({ mode: 'webgpu' });
      let built;
      if (mode.perPass) {
      ...
      // correctness gate before any timing; also the compile warmup
      ...
      log(workload.name, '/', mode.label + ':', ms, 'ms (checksum', checksum.toFixed(6) + ')');
      await gpu.destroy();
    }
    table.push(row);
  }

  return { adapterLabel, userAgent: navigator.userAgent, table };
}

// -- report ----------------------------------------------------------------
function printReport(data, chromiumVersion) {
  const labels = ['webgpu per-pass', 'pipeline generic', 'pipeline fused-encoder'];
  const lines = [];
  lines.push('### Benchmarks — webgpu pipeline lowering');
  lines.push('');
  lines.push('- Chromium ' + chromiumVersion + ' (headed)');
  lines.push('- WebGPU adapter: ' + data.adapterLabel);
  lines.push('- median of 5; every run synced by reading back its own result; checksums validated against the plain-JS oracle before timing');
  lines.push('');
  lines.push(`| Workload | ${ labels.join(' | ') } |`);
  lines.push(`|---|${ labels.map(() => '---').join('|') }|`);
  for (const row of data.table) {
    const baseline = row.modes['webgpu per-pass'].ms;
    lines.push(`| ${ row.name } | ${ labels.map(label => {
      const ms = row.modes[label].ms;
      const speedup = label === 'webgpu per-pass' ? ' (1×)' : ` (${ (baseline / ms).toFixed(2) }×)`;
      return `${ ms } ms${ speedup }`;
    }).join(' | ') } |`);
  }
  lines.push('');
  lines.push(JSON.stringify(data.table, null, 2));
