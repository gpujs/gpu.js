...
test('handles Math.pow with floats', () => {
  ...
  assert.equal(
    node.toString(),
    'float kernel(float user_v, float user_v2) {' +
...
test('handles Math.pow with mixed 1', () => {
  ...
  assert.equal(
    node.toString(),
    'float kernel(float user_v, int user_v2) {' +
...
test('handles Math.pow with mixed 2', () => {
  ...
  assert.equal(
    node.toString(),
    'float kernel(int user_v, float user_v2) {' +
...
test('handles Math.pow with ints', () => {
  ...
  assert.equal(
    node.toString(),
    'float kernel(int user_v, int user_v2) {' +
  ...
  const node = new WebGLFunctionNode(
    'function kernel(v) {' + '\n return childFunction(v);' + '\n}',
    {
      ...
      lookupFunctionArgumentTypes: functionName => {
        ...
        throw new Error(
          `unhanded lookupFunctionArgumentTypes for ${functionName}`
        );
      },
...
test('handles argument of type HTMLImageArray', () => {
  ...
  assert.equal(
    node.toString(),
    'float kernel(sampler2DArray user_v,ivec2 user_vSize,ivec3 user_vDim) {' +
      '\nreturn childFunction(user_v,user_vSize,user_vDim);' +
      '\n}'
  );
  assert.equal(lookupReturnTypeCalls, 2);
