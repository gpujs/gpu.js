...
function getPixelsStandard(mode) {
  ...
  const kernel = gpu.createKernel(
    function (v) {
      this.color(v[this.thread.y][this.thread.x][0], v[this.thread.y][this.thread.x][1], v[this.thread.y][this.thread.x][2]);
    },
    {
      output: [2, 2],
      graphical: true,
...
test('standard auto', () => {
  ...
  assert.deepEqual(pixels, [46, 51, 56, 255, 66, 71, 76, 255, 5, 10, 15, 255, 25, 31, 36, 255]);
});
...
(GPU.isCanvasSupported ? test : skip)('standard cpu', () => {
  ...
  assert.deepEqual(pixels, [45, 51, 56, 255, 66, 71, 76, 255, 5, 10, 15, 255, 25, 30, 35, 255]);
});
...
test('flipped auto', () => {
  ...
  assert.deepEqual(pixels, [5, 10, 15, 255, 25, 31, 36, 255, 46, 51, 56, 255, 66, 71, 76, 255]);
});
...
(GPU.isCanvasSupported ? test : skip)('flipped cpu', () => {
  ...
  assert.deepEqual(pixels, [5, 10, 15, 255, 25, 30, 35, 255, 45, 51, 56, 255, 66, 71, 76, 255]);
});
