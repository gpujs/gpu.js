...
(GPU.isSinglePrecisionSupported ? test : skip)(
  'Issue #349 - divide by three auto',
  () => {
    testDivideByThree();
  }
);
...
(GPU.isSinglePrecisionSupported && GPU.isWebGLSupported ? test : skip)(
  'Issue #349 - divide by three webgl',
  () => {
...
(GPU.isWebGL2Supported ? test : skip)(
  'Issue #349 - divide by three webgl2',
  () => {
...
(GPU.isHeadlessGLSupported ? test : skip)(
  'Issue #349 - divide by three headlessgl',
  () => {
...
function someRandomWholeNumberDivisions(mode) {
  ...
  for (let i = 0; i < DATA_MAX; i++) {
    divisorData[i] = parseInt(
      Math.random() * maxWholeNumberRepresentation + 1,
      10
    );
    expectedResults[i] = parseInt(
      Math.random() * maxWholeNumberRepresentation + 1,
      10
    );
  ...
  assert.ok(
    ...
    same
      ? ''
      : 'not all elements are the same, failed on index:' +
          i +
          ' ' +
          dividendData[i] +
          '/' +
...
(GPU.isSinglePrecisionSupported ? test : skip)(
  'Issue #349 - some random whole number divisions auto',
  () => {
    someRandomWholeNumberDivisions();
  }
);
(GPU.isSinglePrecisionSupported ? test : skip)(
  'Issue #349 - some random whole number divisions gpu',
  () => {
    someRandomWholeNumberDivisions('gpu');
  }
);
(GPU.isSinglePrecisionSupported && GPU.isWebGLSupported ? test : skip)(
  'Issue #349 - some random whole number divisions webgl',
  () => {
    someRandomWholeNumberDivisions('webgl');
  }
);
(GPU.isWebGL2Supported ? test : skip)(
  'Issue #349 - some random whole number divisions webgl2',
  () => {
    someRandomWholeNumberDivisions('webgl2');
  }
);
(GPU.isHeadlessGLSupported ? test : skip)(
  'Issue #349 - some random whole number divisions headlessgl',
  () => {
  ...
  if (!gpu.Kernel.features.isIntegerDivisionAccurate) {
    assert.ok(
      idFix(6, 3)[0] === 2 &&
        ...
        (idDixOff(6, 3)[0] !== 2 || idDixOff(6030401, 3991)[0] !== 1511),
      'when bug is present should show bug!'
    );
  } else {
    assert.ok(
      idFix(6, 3)[0] === 2 && idDixOff(6, 3)[0] === 2,
      "when bug isn't present should not show bug!"
    );
  }
  gpu.destroy();
}
(GPU.isSinglePrecisionSupported ? test : skip)(
  'Issue #349 - test disable fix integer division bug auto',
  () => {
...
(GPU.isSinglePrecisionSupported ? test : skip)(
  'Issue #349 - test disable fix integer division bug gpu',
  () => {
...
(GPU.isSinglePrecisionSupported && GPU.isWebGLSupported ? test : skip)(
  'Issue #349 - test disable fix integer division bug webgl',
  () => {
...
(GPU.isWebGL2Supported ? test : skip)(
  'Issue #349 - test disable fix integer division bug webgl2',
  () => {
...
(GPU.isHeadlessGLSupported ? test : skip)(
  'Issue #349 - test disable fix integer division bug headlessgl',
  () => {
