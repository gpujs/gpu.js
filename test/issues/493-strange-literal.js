...
function testStrangeLiteral(mode) {
  ...
  function kernelFunction(array) {
    ...
    const yFactor = (1 - 0.5) * this.constants.x + this.thread.x * this.constants.y;
    const value = array[this.thread.x];
    return [value[0] / xFactor, value[1] / yFactor];
  }
  const kernel1 = gpu.createKernel(kernelFunction).setArgumentTypes({ array: 'Array1D(2)' }).setConstants({ x: 1, y: 1 }).setOutput([1]);
  assert.deepEqual(kernel1([[1, 2]]), [new Float32Array([1, 4])]);
  const kernel2 = gpu.createKernel(kernelFunction).setStrictIntegers(true).setArgumentTypes({ array: 'Array1D(2)' }).setConstants({ x: 1, y: 1 }).setOutput([1]);
