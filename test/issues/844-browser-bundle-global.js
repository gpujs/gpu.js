...
['gpu-browser.js', 'gpu-browser.min.js'].forEach(file => {
  test(`new GPU() works with native WebGPU present (Chrome/Edge) - ${file}`, () => {
  ...
  test(`new GPU() works without native WebGPU (Firefox) - ${file}`, () => {
  ...
  test(`new GPU.GPU() workaround still works - ${file}`, () => {
    const window = loadBundle(file, nativeWebGPUStub());
  ...
  test(`namespace exports remain attached to GPU - ${file}`, () => {
    const window = loadBundle(file, {});
  ...
  test(`an already loaded gpu.js is left in place - ${file}`, () => {
    const window = loadBundle(file, {});
