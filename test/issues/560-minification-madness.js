...
function testMinificationMadness(mode, canvas) {
  ...
  const kernel = gpu.createKernel(
    function (t, e, i, n, r) {
      for (var o = this.constants.maxIter, a = this.constants.canvasWidth, s = this.constants.canvasHeight, l = i + (n - i) * (this.thread.y / s), c = t + (e - t) * (this.thread.x / a), p = 0, u = 0, h = 0, d = 0; p * p + u * u < 4 && h < o; ) (d = p * p - u * u + c), (u = 2 * p * u + l), (p = d), h++;
      h === o ? this.color(0, 0, 0, 1) : this.color(r[3 * h] / 255, r[3 * h + 1] / 255, r[3 * h + 2] / 255, 1);
    },
    {
      output: [1, 1],
      constants: {
  ...
  if (kernel.context && kernel.context.getError) assert.ok(kernel.context.getError() === 0);
  gpu.destroy();
}
