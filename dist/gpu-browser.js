 ...
 *
 * @version 2.19.8
 * @date Mon Jul 27 2026 12:19:32 GMT+0800 (Singapore Standard Time)
 *
 * @license MIT
 * The MIT License
 *
 ...
 */(function(global, factory) {
  ...
  var require_acorn = __commonJSMin((exports, module) => {
    (function(global, factory) {
      ...
      function getOptions(opts) {
        ...
        if (options.ecmaVersion === "latest") options.ecmaVersion = 1e8; else if (options.ecmaVersion == null) {
      ...
      pp$8.parseTopLevel = function(node) {
        ...
        while (this.type !== types$1.eof) {
          var stmt = this.parseStatement(null, true, exports$2);
          node.body.push(stmt);
        }
      ...
      pp$8.parseStatement = function(context, topLevel, exports$3) {
        ...
        switch (starttype) {
         ...
         case types$1._import:
          ...
          return starttype === types$1._import ? this.parseImport(node) : this.parseExport(node, exports$3);
         ...
         default:
          ...
          if (starttype === types$1.name && expr.type === "Identifier" && this.eat(types$1.colon)) return this.parseLabeledStatement(node, maybeName, expr, context); else return this.parseExpressionStatement(node, expr);
        }
      };
      pp$8.parseBreakContinueStatement = function(node, keyword) {
        var isBreak = keyword === "break";
        this.next();
        if (this.eat(types$1.semi) || this.insertSemicolon()) node.label = null; else if (this.type !== types$1.name) this.unexpected(); else {
      ...
      pp$8.parseDoStatement = function(node) {
        ...
        if (this.options.ecmaVersion >= 6) this.eat(types$1.semi); else this.semicolon();
      ...
      pp$8.parseForStatement = function(node) {
        ...
        if (this.type === types$1._in || (isForOf = this.options.ecmaVersion >= 6 && this.isContextual("of"))) {
          if (awaitAt > -1) {
            ...
            if (init.start === initPos && !containsEsc && init.type === "Identifier" && init.name === "async") this.unexpected(); else if (this.options.ecmaVersion >= 9) node.await = false;
          }
      ...
      pp$8.parseReturnStatement = function(node) {
        ...
        this.next();
        if (this.eat(types$1.semi) || this.insertSemicolon()) node.argument = null; else {
      ...
      pp$8.parseSwitchStatement = function(node) {
        ...
        for (var sawDefault = false; this.type !== types$1.braceR; ) if (this.type === types$1._case || this.type === types$1._default) {
          ...
          cur.consequent = [];
          this.next();
          if (isCase) cur.test = this.parseExpression(); else {
      ...
      pp$8.parseTryStatement = function(node) {
        ...
        if (this.type === types$1._catch) {
          ...
          this.next();
          if (this.eat(types$1.parenL)) clause.param = this.parseCatchClauseParam(); else {
      ...
      pp$8.parseVar = function(node, isFor, kind, allowMissingInitializer) {
        ...
        for (;;) {
          ...
          if (this.eat(types$1.eq)) decl.init = this.parseMaybeAssign(isFor); else if (!allowMissingInitializer && kind === "const" && !(this.type === types$1._in || this.options.ecmaVersion >= 6 && this.isContextual("of"))) this.unexpected(); else if (!allowMissingInitializer && decl.id.type !== "Identifier" && !(isFor && (this.type === types$1._in || this.isContextual("of")))) this.raise(this.lastTokEnd, "Complex binding patterns require an initialization value"); else decl.init = null;
      ...
      pp$8.parseClassElement = function(constructorAllowsSuper) {
        ...
        if (this.eatContextual("static")) {
          ...
          if (this.isClassElementNameStart() || this.type === types$1.star) isStatic = true; else keyName = "static";
        }
        node.static = isStatic;
        if (!keyName && ecmaVersion >= 8 && this.eatContextual("async")) if ((this.isClassElementNameStart() || this.type === types$1.star) && !this.canInsertSemicolon()) isAsync = true; else keyName = "async";
        if (!keyName && (ecmaVersion >= 9 || !isAsync) && this.eat(types$1.star)) isGenerator = true;
        if (!keyName && !isAsync && !isGenerator) {
          var lastValue = this.value;
          if (this.eatContextual("get") || this.eatContextual("set")) if (this.isClassElementNameStart()) kind = lastValue; else keyName = lastValue;
        }
        if (keyName) {
          node.computed = false;
      ...
      pp$8.parseClassField = function(field) {
        if (checkKeyName(field, "constructor")) this.raise(field.key.start, "Classes can't have a field named 'constructor'"); else if (field.static && checkKeyName(field, "prototype")) this.raise(field.key.start, "Classes can't have a static field named 'prototype'");
      ...
      pp$8.exitClassBody = function() {
        ...
        for (var i = 0; i < used.length; ++i) {
          var id = used[i];
          if (!hasOwn(declared, id.name)) if (parent) parent.used.push(id); else this.raiseRecoverable(id.start, "Private field '#" + id.name + "' must be declared in an enclosing class");
        }
      };
      ...
      pp$8.parseExport = function(node, exports$5) {
        this.next();
        if (this.eat(types$1.star)) return this.parseExportAllDeclaration(node, exports$5);
        if (this.eat(types$1._default)) {
          this.checkExport(exports$5, "default", this.lastTokStart);
          node.declaration = this.parseExportDefaultDeclaration();
          return this.finishNode(node, "ExportDefaultDeclaration");
        }
        if (this.shouldParseExportStatement()) {
          node.declaration = this.parseExportDeclaration(node);
          if (node.declaration.type === "VariableDeclaration") this.checkVariableExport(exports$5, node.declaration.declarations); else this.checkExport(exports$5, node.declaration.id, node.declaration.id.start);
          node.specifiers = [];
          node.source = null;
        } else {
          node.declaration = null;
          node.specifiers = this.parseExportSpecifiers(exports$5);
      ...
      pp$8.checkExport = function(exports$6, name, pos) {
        ...
        if (hasOwn(exports$6, name)) this.raiseRecoverable(pos, "Duplicate export '" + name + "'");
        exports$6[name] = true;
      };
      pp$8.checkPatternExport = function(exports$7, pat) {
        var type = pat.type;
        if (type === "Identifier") this.checkExport(exports$7, pat, pat.start); else if (type === "ObjectPattern") for (var i = 0, list = pat.properties; i < list.length; i += 1) {
          var prop = list[i];
          ...
          var elt = list$1[i$1];
          if (elt) this.checkPatternExport(exports$7, elt);
        } else if (type === "Property") this.checkPatternExport(exports$7, pat.value); else if (type === "AssignmentPattern") this.checkPatternExport(exports$7, pat.left); else if (type === "RestElement") this.checkPatternExport(exports$7, pat.argument);
      };
      ...
      pp$8.parseExportSpecifier = function(exports$9) {
        ...
        this.checkExport(exports$9, node.exported, node.exported.start);
      ...
      pp$8.parseImportSpecifier = function() {
        ...
        if (this.eatContextual("as")) node.local = this.parseIdent(); else {
      ...
      pp$7.parseBindingList = function(close, allowEmpty, allowTrailingComma, allowModifiers) {
        ...
        while (!this.eat(close)) {
          if (first) first = false; else this.expect(types$1.comma);
          if (allowEmpty && this.type === types$1.comma) elts.push(null); else if (allowTrailingComma && this.afterTrailingComma(close)) break; else if (this.type === types$1.ellipsis) {
      ...
      pp$6.updateContext = function(prevType) {
        ...
        if (type.keyword && prevType === types$1.dot) this.exprAllowed = false; else if (update = type.updateContext) update.call(this, prevType); else this.exprAllowed = type.beforeExpr;
      };
      ...
      types$1._function.updateContext = types$1._class.updateContext = function(prevType) {
        if (prevType.beforeExpr && prevType !== types$1._else && !(prevType === types$1.semi && this.curContext() !== types.p_stat) && !(prevType === types$1._return && lineBreak.test(this.input.slice(this.lastTokEnd, this.start))) && !((prevType === types$1.colon || prevType === types$1.braceL) && this.curContext() === types.b_stat)) this.context.push(types.f_expr); else this.context.push(types.f_stat);
      ...
      pp$5.checkPropClash = function(prop, propHash, refDestructuringErrors) {
        ...
        if (other) {
          var redefinition;
          if (kind === "init") redefinition = this.strict && other.init || other.get || other.set; else redefinition = other.init || other[kind];
      ...
      pp$5.parseMaybeAssign = function(forInit, refDestructuringErrors, afterLeftParse) {
        if (this.isContextual("yield")) if (this.inGenerator) return this.parseYield(forInit); else this.exprAllowed = false;
        ...
        if (this.type.isAssign) {
          ...
          if (this.type === types$1.eq) this.checkLValPattern(left); else this.checkLValSimple(left);
          node.left = left;
          this.next();
      ...
      pp$5.parseMaybeUnary = function(refDestructuringErrors, sawUnary, incDec, forInit) {
        ...
        if (!incDec && this.eat(types$1.starstar)) if (sawUnary) this.unexpected(this.lastTokStart); else return this.buildBinary(startPos, startLoc, expr, this.parseMaybeUnary(null, false, false, forInit), "**", false); else return expr;
      };
      ...
      pp$5.parseSubscript = function(base, startPos, startLoc, noCalls, maybeAsyncArrow, optionalChained, forInit) {
        ...
        if (computed || optional && this.type !== types$1.parenL && this.type !== types$1.backQuote || this.eat(types$1.dot)) {
          ...
          } else if (this.type === types$1.privateId && base.type !== "Super") node.property = this.parsePrivateIdent(); else node.property = this.parseIdent(this.options.allowReserved !== "never");
      ...
      pp$5.parseExprAtom = function(refDestructuringErrors, forInit, forNew) {
        ...
        switch (this.type) {
         ...
         case types$1._import:
          if (this.options.ecmaVersion >= 11) return this.parseExprImport(forNew); else return this.unexpected();
      ...
      pp$5.parseExprImport = function(forNew) {
        ...
        this.next();
        if (this.type === types$1.parenL && !forNew) return this.parseDynamicImport(node); else if (this.type === types$1.dot) {
      ...
      pp$5.parseDynamicImport = function(node) {
        ...
        } else node.options = null; else if (!this.eat(types$1.parenR)) {
          var errorPos = this.start;
          if (this.eat(types$1.comma) && this.eat(types$1.parenR)) this.raiseRecoverable(errorPos, "Trailing comma is not allowed in import()"); else this.unexpected(errorPos);
        }
      ...
      pp$5.parseNew = function() {
        ...
        if (this.eat(types$1.parenL)) node.arguments = this.parseExprList(types$1.parenR, this.options.ecmaVersion >= 8, false); else node.arguments = empty;
      ...
      pp$5.parsePropertyValue = function(prop, isPattern, isGenerator, isAsync, startPos, startLoc, refDestructuringErrors, containsEsc) {
        ...
        if (this.eat(types$1.colon)) {
          ...
          prop.kind = "init";
          if (isPattern) prop.value = this.parseMaybeDefault(startPos, startLoc, this.copyNode(prop.key)); else if (this.type === types$1.eq && refDestructuringErrors) {
      ...
      pp$5.parseExprList = function(close, allowTrailingComma, allowEmpty, refDestructuringErrors) {
        ...
        while (!this.eat(close)) {
          ...
          var elt = void 0;
          if (allowEmpty && this.type === types$1.comma) elt = null; else if (this.type === types$1.ellipsis) {
      ...
      pp$5.parseIdentNode = function() {
        var node = this.startNode();
        if (this.type === types$1.name) node.name = this.value; else if (this.type.keyword) {
      ...
      pp$5.parsePrivateIdent = function() {
        var node = this.startNode();
        if (this.type === types$1.privateId) node.name = this.value; else this.unexpected();
        this.next();
        this.finishNode(node, "PrivateIdentifier");
        if (this.options.checkPrivateFields) if (this.privateNameStack.length === 0) this.raise(node.start, "Private field '#" + node.name + "' must be declared in an enclosing class"); else this.privateNameStack[this.privateNameStack.length - 1].used.push(node);
        return node;
      };
      ...
      pp$3.declareName = function(name, bindingType, pos) {
        ...
        if (bindingType === BIND_LEXICAL) {
          ...
          if (this.treatFunctionsAsVar) redeclared = scope$2.lexical.indexOf(name) > -1; else redeclared = scope$2.lexical.indexOf(name) > -1 || scope$2.var.indexOf(name) > -1;
      ...
      pp$1.regexp_eatCapturingGroup = function(state) {
        if (state.eat(40)) {
          if (this.options.ecmaVersion >= 9) this.regexp_groupSpecifier(state); else if (state.current() === 63) state.raise("Invalid group");
          this.regexp_disjunction(state);
          if (state.eat(41)) {
      ...
      pp$1.regexp_groupSpecifier = function(state) {
        if (state.eat(63)) {
          ...
          if (trackDisjunction) (known || (state.groupNames[state.lastStringValue] = [])).push(state.branchID); else state.groupNames[state.lastStringValue] = true;
        }
      };
      ...
      pp$1.regexp_eatLegacyOctalEscapeSequence = function(state) {
        if (this.regexp_eatOctalDigit(state)) {
          ...
          if (this.regexp_eatOctalDigit(state)) {
            ...
            if (n1 <= 3 && this.regexp_eatOctalDigit(state)) state.lastIntValue = n1 * 64 + n2 * 8 + state.lastIntValue; else state.lastIntValue = n1 * 8 + n2;
      ...
      pp.skipSpace = function() {
        loop: while (this.pos < this.input.length) {
          ...
          switch (ch) {
           ...
           default:
            if (ch > 8 && ch < 14 || ch >= 5760 && nonASCIIwhitespace.test(String.fromCharCode(ch))) ++this.pos; else break loop;
          }
        }
      };
      ...
      pp.tryReadTemplateToken = function() {
        ...
        try {
          ...
          if (err === INVALID_TEMPLATE_ESCAPE_ERROR) this.readInvalidTemplateToken(); else throw err;
        }
        this.inTemplateElement = false;
      };
      pp.invalidStringToken = function(position, message) {
        if (this.inTemplateElement && this.options.ecmaVersion >= 9) throw INVALID_TEMPLATE_ESCAPE_ERROR; else this.raise(position, message);
      };
      ...
      pp.readWord1 = function() {
        ...
        while (this.pos < this.input.length) {
          ...
          if (isIdentifierChar(ch, astral)) this.pos += ch <= 65535 ? 1 : 2; else if (ch === 92) {
  ...
  var require_function_node$2 = __commonJSMin((exports, module) => {
    const {FunctionNode: FunctionNode} = require_function_node$3();
  ...
  var require_kernel$4 = __commonJSMin((exports, module) => {
    const {Kernel: Kernel} = require_kernel$5();
    const {FunctionBuilder: FunctionBuilder} = require_function_builder();
    const {CPUFunctionNode: CPUFunctionNode} = require_function_node$2();
    const {utils: utils} = require_utils();
  ...
  var require_kernel$2 = __commonJSMin((exports, module) => {
    const {GLKernel: GLKernel} = require_kernel$3();
    const {FunctionBuilder: FunctionBuilder} = require_function_builder();
    const {WebGLFunctionNode: WebGLFunctionNode} = require_function_node$1();
    const {utils: utils} = require_utils();
  ...
  var require_kernel$1 = __commonJSMin((exports, module) => {
    ...
    const {WebGLKernel: WebGLKernel} = require_kernel$2();
    const {glKernelString: glKernelString} = require_kernel_string();
    let isSupported = null;
    let testCanvas = null;
    let testContext = null;
  ...
  var require_kernel = __commonJSMin((exports, module) => {
    const {WebGLKernel: WebGLKernel} = require_kernel$2();
    const {WebGL2FunctionNode: WebGL2FunctionNode} = require_function_node();
    const {FunctionBuilder: FunctionBuilder} = require_function_builder();
    const {utils: utils} = require_utils();
  ...
  var require_gpu = __commonJSMin((exports, module) => {
    ...
    const {utils: utils} = require_utils();
    const {Kernel: Kernel} = require_kernel$5();
    const {CPUKernel: CPUKernel} = require_kernel$4();
    const {HeadlessGLKernel: HeadlessGLKernel} = require_kernel$1();
    const {WebGL2Kernel: WebGL2Kernel} = require_kernel();
    const {WebGLKernel: WebGLKernel} = require_kernel$2();
