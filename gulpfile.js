...
gulp.task('build', function() {
	return true &&
		...
		.pipe(gulp.dest('bin')) &&
		// PrecompiledGPU.js sub files
		gulp.src([
			'src/utils.js',
			'src/precompiled/PrecompiledGpu.js',
			'src/precompiled/suffix.js'
		])
		.pipe(concat('PrecompiledGPU.js'))
		.pipe(header(fs.readFileSync('src/precompiled/prefix.js', 'utf8'), { pkg : pkg } ))
		.pipe(gulp.dest('bin'));
});
...
gulp.task('minify', ['build'], function() {
	return true &&
		...
		.pipe(gulp.dest('bin')) &&
		// PrecompiledGPU.js 
		gulp.src(['bin/PrecompiledGPU.js'])
		.pipe(rename('PrecompiledGPU.min.js'))
		.pipe(uglify({
			mangle: false,
			preserveComments: "license"
		}).on('error', gutil.log))
